...
# Use a wall-clock wait (ms) for GPU instead of frame counts
const GPU_WAIT_MS: int = 50

# Single timer settings (frame-rate independent)
@export var queue_interval: float = 0.05        # seconds between timer ticks (20 Hz)
@export var adds_per_tick: int = 2              # how many chunks to instantiate per tick
@export var removes_per_tick: int = 1           # how many chunks to remove per tick
var single_timer: Timer
...
func _ready() -> void:
	...
	# Single timer to handle both compute polling and queue processing
	single_timer = Timer.new()
	single_timer.wait_time = queue_interval
	single_timer.one_shot = false
	single_timer.autostart = true
	add_child(single_timer)
	single_timer.connect("timeout", Callable(self, "_on_single_timer_timeout"))


func _process(delta: float) -> void:
	# keep only cheap, per-frame operations here (player position checks, idle timers)
	if not player:
		return
...
func check_compute_status() -> void:
	if hotseat:
		if waiting_for_compute and OS.get_ticks_msec() - hotseat.submit_time_ms >= GPU_WAIT_MS:
...
func startup_compute() -> void:
	...
	# record wall-clock submit time (ms)
	hotseat["submit_time_ms"] = OS.get_ticks_msec()
...
# Single timer handler: poll compute and process queues
func _on_single_timer_timeout() -> void:
	# 1) Poll compute/GPU/mesh status and advance the hotseat state
	check_compute_status()

	# 2) Process a small number of adds per tick (keeps work spread over time)
	var adds = 0
	while adds < adds_per_tick and add_queue.size() > 0:
		var coord = add_queue.pop()
		add_lookup.erase(coord)
		if active_chunks.has(coord):
			continue
	...
	# 3) Process a small number of removes per tick
	var removes = 0
	while removes < removes_per_tick and remove_queue.size() > 0:
		var coord = remove_queue.pop()
		remove_lookup.erase(coord)
		if not active_chunks.has(coord):
			continue
