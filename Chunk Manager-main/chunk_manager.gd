...
## Checking Compute VS Mesh time
## use "compute_time_ms" for compute
## use a "start" and "end" time for mesh
##
## if compute > mesh, GPU is the bottleneck
## if compute < mesh, meshthread is the bottleneck
## if compute ~= mesh, balance is good; good candidicy for parallelization
...
func check_nearby_chunks(current_chunk: Vector2i) -> void:
	for x in range(-chunk_load_radius, chunk_load_radius + 1):
		for z in range(-chunk_load_radius, chunk_load_radius + 1):
			...
			if coord.distance_to(current_chunk) <= chunk_load_radius:
				add_chunk_to_queue(coord)


func add_chunk_to_queue(coord: Vector2i) -> void:
	if active_chunks.has(coord):
		return
